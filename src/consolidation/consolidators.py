...
class DataConsolidator:
    ...
    def discover_parquet_files(self) -> List[Path]:
        ...
        parquet_files = list(
            self.processed_data_path.glob("aerogeradores_processed_*.parquet")
        )
        parquet_files.sort()  # Ordenar por nome
        logger.info(
            f"Descobertos {len(parquet_files)} arquivos Parquet para consolidar"
        )
    ...
    def check_consolidation_needed(self) -> dict:
        ...
        if not parquet_files:
            return {
                ...
                "reason": "Nenhum arquivo Parquet encontrado",
            }
        ...
        try:
            ...
            if csv_records == expected_records:
                ...
                logger.warning(
                    f"CSV tem mais registros ({csv_records}) que parquets ({expected_records})"
                )
                return {
                    "needs_consolidation": True,
            ...
            return {"needs_consolidation": True, "reason": f"Erro ao ler CSV: {e}"}
    ...
    def load_and_combine_parquets(self, parquet_files: List[Path]) -> pd.DataFrame:
        ...
        if not parquet_files:
            raise DataProcessingError(
                "Nenhum arquivo Parquet encontrado para consolidar"
            )
        ...
        if len(consolidated_df) != total_records:
            logger.warning(
                f"Possível perda de registros: {total_records} → {len(consolidated_df)}"
            )
    ...
    def clean_and_optimize_for_tableau(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "DATA_ATUALIZACAO" in df_clean.columns:
            try:
                logger.info(
                    "Convertendo DATA_ATUALIZACAO de timestamp Unix para formato de data..."
                )
                ...
                df_clean["DATA_ATUALIZACAO"] = df_clean["DATA_ATUALIZACAO"].dt.strftime(
                    "%Y-%m-%d"
                )

                logger.info(
                    "✅ DATA_ATUALIZACAO convertida de timestamp Unix para formato de data"
                )
        ...
        if "OPERACAO" in df_clean.columns:
            ...
            logger.info(f"   Registros depois: {registros_depois:,}")
            logger.info(
                f"   Registros removidos: {registros_removidos:,} (null, 1, etc)"
            )
        ...
        logger.info(
            f"Otimização concluída: {len(df_clean)} registros, {len(df_clean.columns)} colunas"
        )
        return df_clean
    ...
    def consolidate_all(
        self, output_filename: Optional[str] = None, force_refresh: bool = False
    ) -> str:
        ...
        if not force_refresh:
            ...
            if not check_result["needs_consolidation"]:
                ...
                if "csv_records" in check_result:
                    ...
                    logger.info(
                        f"Registros esperados: {check_result['expected_records']:,}"
                    )
                ...
                if "csv_records" in check_result and "expected_records" in check_result:
                    ...
                    logger.info(
                        f"Esperado: {check_result['expected_records']:,} registros"
                    )
        else:
    ...
    def cleanup_all_output_data(self):
        ...
        try:
            old_csv_files = list(
                self.output_data_path.glob("aerogeradores_consolidado_*.csv")
            )
            ...
            logger.info(f"Limpeza manual: {removed_count} CSVs removidos")
